...
function getScraper(search_engine, args) {
  if (typeof search_engine === "string") {
    ...
    throw new Error(
      `search_engine must either be a string of class (function)`
    );
  }
}
...
class ScrapeManager {
  constructor(config, context = {}) {
    ...
    this.config = {
      ...
      // settings for ./puppeteer-cluster/dist/index.js
    ...
    this.config.search_engine_name =
      typeof this.config.search_engine === "function"
        ? this.config.search_engine.name
        : this.config.search_engine;
    ...
    if (fs.existsSync(this.config.proxy_file)) {
      ...
      log(
        this.config,
        1,
        `${this.config.proxies.length} proxies read from file.`
      );
    }
  ...
  async start() {
    ...
    if (
      Array.isArray(this.config.chrome_flags) &&
      this.config.chrome_flags.length
    ) {
    ...
    if (user_agent) {
      chrome_flags.push(`--user-agent=${user_agent}`);
    }
    ...
    if (this.config.proxy) {
      ...
      chrome_flags.push("--proxy-server=" + this.config.proxy);
    }
    ...
    log(
      this.config,
      2,
      `Using the following puppeteer configuration: ${launch_args}`
    );
    ...
    if (this.pluggable) {
      ...
      // use ./puppeteer-cluster/dist/index.js for scraping
      const { Cluster } = require("./puppeteer-cluster/dist/index.js");
      ...
      if (this.config.proxies.length > 0) {
        this.config.puppeteer_cluster_config.concurrency =
  ...
  async scrape(scrape_config = {}) {
    if (!scrape_config.keywords && !scrape_config.keyword_file) {
      console.error(
        "Either keywords or keyword_file must be supplied to scrape()"
      );
      return false;
    }
    ...
    if (this.config.keywords && this.config.search_engine) {
      log(
        this.config,
        1,
        `[se-scraper] started at [${new Date().toUTCString()}] and scrapes ${
          this.config.search_engine_name
        } with ${this.config.keywords.length} keywords on ${
          this.config.num_pages
        } pages each.`
      );
    }
