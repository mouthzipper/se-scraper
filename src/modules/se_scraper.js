    ...
    for (var keyword of this.keywords) {
      ...
      try {
        ...
        log(
          this.config,
          1,
          `${this.config.search_engine_name} scrapes keyword "${keyword}" on page ${this.page_num}`
        );
        ...
        console.log("Sucessfully moved to the next page");
        ...
        console.error(
          `Problem with scraping ${keyword} in search engine ${this.config.search_engine_name}: ${e}`
        );
        ...
        if (this.config.debug_level > 2) {
          try {
            ...
            await this.page.screenshot({
              path: `debug_se_scraper_${this.config.search_engine_name}_${keyword}.png`
            });
          } catch (e) {}
        }
        ...
        if ((await this.detected()) === true) {
          console.error(
            `${this.config.search_engine_name} detected the scraping!`
          );
    ...
    for (let needle of needles) {
      if (html.includes(needle)) {
        console.log(
          this.config,
          2,
          `HTML contains needle ${needle}. no_results=true`
        );
        return true;
      }
    }
